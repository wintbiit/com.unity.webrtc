...
namespace webrtc
{
    ...
    NvDecoderImplH264::NvDecoderImplH264(CUcontext context, ProfilerMarkerFactory* profiler)
        : m_context(context)
        , m_decoder(nullptr)
    ...
    NvDecoderImplH264::~NvDecoderImplH264() { Release(); }

    VideoDecoder::DecoderInfo NvDecoderImplH264::GetDecoderInfo() const
    {
    ...
    bool NvDecoderImplH264::Configure(const Settings& settings)
    {
    ...
    int32_t NvDecoderImplH264::RegisterDecodeCompleteCallback(DecodedImageCallback* callback)
    {
        this->m_decodedCompleteCallback = callback;
        return WEBRTC_VIDEO_CODEC_OK;
    }

    int32_t NvDecoderImplH264::Release()
    {
        m_buffer_pool.Release();
        return WEBRTC_VIDEO_CODEC_OK;
    }

    int32_t NvDecoderImplH264::Decode(const EncodedImage& input_image, bool missing_frames, int64_t render_time_ms)
    {
        CUcontext current;
