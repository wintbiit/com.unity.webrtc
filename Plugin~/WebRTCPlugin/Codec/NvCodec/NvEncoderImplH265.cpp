...
namespace webrtc
{
    std::optional<H265Level> NvEncoderImplH265::s_maxSupportedH265Level;
    std::vector<SdpVideoFormat> NvEncoderImplH265::s_formats;

    NvEncoderImplH265::NvEncoderImplH265(
        ...
        , m_profiler(profiler)
    {
        ...
        RTC_CHECK(codec.GetParam(cricket::kH265FmtpProfileId, &profileId));
        std::string levelId;
        RTC_CHECK(codec.GetParam(cricket::kH265FmtpLevelId, &levelId));
        std::string tierFlag;
        RTC_CHECK(codec.GetParam(cricket::kH265FmtpTierFlag, &tierFlag));

        auto profile = StringToH265Profile(profileId);
        auto level = StringToH265Level(levelId);
        auto tier = StringToH265Tier(tierFlag);

        m_profileGuid = H265ProfileToGuid(profile.value()).value();
        m_level = level.value();
        m_tier = tier.value();
        ...
        // SupportedNvEncoderCodecs and SupportedMaxH265Level function consume the session of NvEnc and the number of
        // the sessions is limited by NVIDIA device. So it caches the return value here.
        if (s_formats.empty())
            s_formats = SupportedNvEncoderCodecs(m_context);
        if (!s_maxSupportedH265Level.has_value())
            s_maxSupportedH265Level = SupportedMaxH265Level(m_context);
    }

    NvEncoderImplH265::~NvEncoderImplH265() { Release(); }

    VideoEncoder::EncoderInfo NvEncoderImplH265::GetEncoderInfo() const
    {
    ...
    int NvEncoderImplH265::InitEncode(const VideoCodec* codec, const VideoEncoder::Settings& settings)
    {
        if (!codec || codec->codecType != kVideoCodecH265)
        {
        ...
        {
            // workaround
            // Use supported max framerate that calculated by H265 level define.
            m_codec.maxFramerate = static_cast<uint32_t>(
                unity::webrtc::SupportedMaxFramerate(s_maxSupportedH265Level.value(), m_codec.width * m_codec.height));
        ...
        if (requiredLevel.value() > static_cast<int32_t>(m_level))
        {
            m_level = static_cast<H265Level>(requiredLevel.value());
        }
        ...
        // m_configurations[0].key_frame_interval = m_codec.H265()->keyFrameInterval;
    ...
    int32_t NvEncoderImplH265::RegisterEncodeCompleteCallback(EncodedImageCallback* callback)
    {
        m_encodedCompleteCallback = callback;
        return WEBRTC_VIDEO_CODEC_OK;
    }

    int32_t NvEncoderImplH265::Release()
    {
        if (m_encoder)
        {
    ...
    bool NvEncoderImplH265::CopyResource(
    ...
    int32_t NvEncoderImplH265::Encode(const ::webrtc::VideoFrame& frame, const std::vector<VideoFrameType>* frameTypes)
    {
        if (!m_encoder)
    ...
    int32_t NvEncoderImplH265::ProcessEncodedFrame(std::vector<uint8_t>& packet, const ::webrtc::VideoFrame& inputFrame)
    {
        ...
        std::vector<H265::NaluIndex> naluIndices = H265::FindNaluIndices(rtc::ArrayView<uint8_t> (packet.data(), packet.size()));
        ...
        m_h265BitstreamParser.ParseBitstream(m_encodedImage);
        m_encodedImage.qp_ = m_h265BitstreamParser.GetLastSliceQp().value_or(-1);

        CodecSpecificInfo codecInfo;
        codecInfo.codecType = kVideoCodecH265;
        // codecInfo.codecSpecific.H265.packetization_mode = H265PacketizationMode::NonInterleaved;
    ...
    void NvEncoderImplH265::SetRates(const RateControlParameters& parameters)
    {
        ...
        // workaround:
        // Use required level if the profile level is lower than required level.
        if (requiredLevel.value() > static_cast<uint32_t>(m_level))
        {
            m_level = static_cast<H265Level>(requiredLevel.value());
        }
        ...
        reInitCodecConfig.encodeCodecConfig.hevcConfig.level = static_cast<uint32_t>(m_level);
    ...
    void NvEncoderImplH265::LayerConfig::SetStreamState(bool sendStream)
    {
